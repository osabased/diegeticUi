...
- Policy: required — the Rokit declaration, selected executable, generated headers, schema, and parent resource record can drift independently.
- Installed-state check: Read `rokit.toml`, run `rokit list 1Axen/blink`, and confirm `blink --version` begins with `Blink 1.0.0-pre.8`. Confirm every generated module header says `generated by blink 1.0.0-pre.8`, then run the canonical verifier.
- Expected identity/state: Resource slug `1axen-blink`, canonical source `https://github.com/1Axen/blink`, Rokit tool `1Axen/blink@1.0.0-pre.8`, and reviewed state `1.0.0-pre.8 / v1.0.0-pre.8 / 324caf3aec16c209d9d31e335e321783fb89256f`.
- Parent-state check: Resolve the affected Roblox project root, then read the matching schema-version 3 record at `.agents/roblox/resources/records/1axen-blink.yaml` and resource-bound learnings under `.agents/roblox/resources/learnings/`; without a project root, use `~/.roblox-resources/records/1axen-blink.yaml` and `~/.roblox-resources/learnings/`. Match the resource slug plus canonical URL and package identity, and stop on a current `blocked_use_or_version`.
- Mismatch/unknown action: Stop version-sensitive work and invoke `roblox-resource-acquisition` in `repair/reconcile` mode.
- Defect handoff: Capture the task, installed identity and version, expected behavior, observed behavior, and smallest reproduction, including the schema, exact command, generated header, and Studio realm; then invoke `roblox-resource-acquisition` in `repair/reconcile` mode.
...
The 1.0.0-pre.8 generator emits newer Luau syntax and its own formatting. Keep the generated files in the narrow exclusions in `selene.toml` and `.styluaignore`; validate them through exact regeneration, the Rojo build, and a Studio playtest. Do not format them, because that creates permanent codegen drift.
