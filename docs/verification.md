...
The verifier builds `.verify/diegeticUi.rbxlx`, then invokes `scripts/run-studio-tests.luau` to launch the isolated `studio-shutdown` and `studio-autosave` suites followed by the ordinary `studio` suite, each in a fresh Studio process under the outer diagnostic guard. Shutdown closes ProfileStore process state; the autosave suite sets its process-global interval to 15 seconds and uses `lest-autosave.toml` for a longer per-test timeout. The ordinary suite retains its five-second timeout. The verifier removes only that exact place file afterward. Guard failures retain the complete process streams and attributed problems beneath `.verify/studio-diagnostics/`; Lest also retains `.lest/studio-run.luau` and `.lest/studio-output.log` when Studio startup or a test fails.
